...
export type DecideOutcome =
  | { ok: true; decision: RoutingDecision; runner: Runner }
...
export class WorkerRouter {
  ...
  async decide(input: DecideInput): Promise<DecideOutcome> {
    if (input.manual !== undefined) return this.manual(input.manual)
    ...
    if (input.context.amplificationProfile === 'boss-only') {
      return this.bossOnly(candidateInput)
    }
    ...
    if (byRule !== null) return this.resolve(byRule)
    ...
    if (classifier !== undefined) {
      ...
      if (runner !== undefined) {
        ...
        if (outcome.decision !== null) return this.resolve(outcome.decision)
      }
    }
    ...
    if (fallback === null) {
      return {
        ok: false,
        error: 'Bu task üçün uyğun işçi tapılmadı (qabiliyyət və ya kontekst limiti çatmır)',
      }
    }
    ...
    return this.resolve({
      ...
      ...(spent.costUsd !== undefined ? { decisionCostUsd: spent.costUsd } : {}),
      reason:
        spent.tokens > 0
  ...
  private manual(manual: { runnerId: string; modelId: string }): DecideOutcome {
  ...
  private bossOnly(candidateInput: {
    db: Db
    runners: ReadonlyMap<string, Runner>
    isRunnerReady?: (runnerId: string) => boolean
  }): DecideOutcome {
